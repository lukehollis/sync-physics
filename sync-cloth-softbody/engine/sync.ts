...
export class SyncConcept {
    ...
    register(syncs: SyncFunctionMap) {
        // Ensure syncs is a valid object
        if (!syncs || typeof syncs !== 'object') {
            console.error('Invalid syncs object provided to register:', syncs);
            return;
        }
        
        // Use for...in loop to avoid Object.keys/entries compatibility issues
    ...
    async synchronize(record: ActionRecord) {
        ...
        if (this.logging === Logging.TRACE) {
            ...
            const boundName = boundAction
                ...
                : "UNDEFINED";
            console.log(
                `${record.concept.constructor.name}.${boundName} ${
                    inspect(record.input)
