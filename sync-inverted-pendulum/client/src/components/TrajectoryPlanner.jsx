...
export default function TrajectoryPlanner({ isOpen, onClose, onLaunchMission }) {
  ...
  const calculatePorkchop = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8002/api/trajectory/porkchop', {
        method: 'POST',
  ...
  const calculateTransfer = async (point) => {
    try {
      const response = await fetch('http://localhost:8002/api/trajectory/calculate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          departure,
  ...
  const handleLaunchMission = async (transfer) => {
    try {
      const response = await fetch('http://localhost:8002/api/mission/launch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
